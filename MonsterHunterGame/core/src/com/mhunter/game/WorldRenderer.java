...
public class WorldRenderer {
    ...
    public void render() {
        ...
        for (int i = 0; i < gc.getMonsterController().getActiveList().size(); i++) {
            GameCharacters gameCharacters = gc.getMonsterController().getActiveList().get(i);
            int cx = gameCharacters.getCellX();
            int cy = gameCharacters.getCellY();
            if (cx >= left && cx < right && cy >= bottom && cy < top) {
                drawables[cy - bottom].add(gameCharacters);
            }
        }
        {
        ...
        for (int y = top - 1; y >= bottom; y--) {
            for (int x = left; x < right; x++) {
                for (int i = 0; i < drawables[y - bottom].size(); i++) {
                    drawables[y - bottom].get(i).render(batch, font16);
                }
            }
            for (int x = left; x < right; x++) {
